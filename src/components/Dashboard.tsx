...
const StatCard = ({ title, value, icon: Icon, color, trend, pulse }: any) => (
  <div className="bg-card p-8 rounded-[2.5rem] shadow-sm border border-border hover:shadow-xl transition-all relative overflow-hidden group">
    ...
    <div className="flex items-center justify-between mb-8">
      <div className={`p-4 rounded-2xl ${color === 'teal' ? 'bg-primary/10 text-primary' : color === 'indigo' ? 'bg-info/10 text-info' : color === 'blue' ? 'bg-info/10 text-info' : 'bg-warning/10 text-warning'} group-hover:scale-110 transition-transform`}>
        <Icon size={24} />
      </div>
      {trend && (
        <span className={`text-[10px] font-black flex items-center space-x-1 ${trend.positive ? 'text-success' : 'text-destructive'}`}>
    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        ...
        <div className="flex gap-4 w-full md:w-auto">
          <Link to="/transparency" className="flex-1 md:flex-none px-8 py-4 bg-card border border-border rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-muted flex items-center justify-center gap-3 transition-all shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-card p-12 rounded-[4rem] shadow-sm border border-border">
          <div className="flex items-center justify-between mb-12">
            <div>
              ...
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-2">Fluxo de Desembolso Financeiro MROSC</p>
            </div>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data}>
                ...
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))', fontWeight: 700 }} />
                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))', fontWeight: 700 }} />
                <Tooltip cursor={{ fill: 'hsl(var(--muted))' }} contentStyle={{ borderRadius: '24px', border: 'none', boxShadow: '0 25px 50px -12px rgb(0 0 0 / 0.2)' }} />
        ...
        <div className="bg-card p-12 rounded-[4rem] shadow-sm border border-border flex flex-col items-center justify-center">
          <h3 className="text-xl font-black text-foreground tracking-tight mb-10 self-start italic">Status de Contas</h3>
      ...
      {!isOSC && (
        <div className="bg-sidebar-background p-12 rounded-[4rem] text-sidebar-foreground flex flex-col md:flex-row items-center justify-between gap-12 relative overflow-hidden group shadow-2xl">
          ...
          <div className="max-w-xl relative z-10">
             ...
             <Link to="/logs" className="px-10 py-5 bg-sidebar-accent hover:opacity-90 rounded-2xl text-[10px] font-black uppercase tracking-widest inline-flex items-center gap-3 transition-all">
